...
public class UpdateChecker {
    ...
    public UpdateChecker(Atlas plugin) {
        this.plugin = plugin;
    }
        ...
        CompletableFuture.supplyAsync(() -> {
                    try {
                        ...
                        HttpRequest request = HttpRequest.newBuilder()
                                .uri(URI.create("https://api.github.com/repos/4w3Offical/Atlas/releases/latest"))
                                .build();
                        ...
                        if (isNewer(currentVersion, tagName)) {
                            plugin.getPluginLogger()
                                    .warn("A new version is available! (Current: " + currentVersion + " | Latest: "
                                            + tagName + ")");
                            plugin.getPluginLogger()
                                    .warn("Download at: https://github.com/4w3Offical/Atlas/releases/latest");
                        }
                    }
                });
    }
