...
import dev.awe.atlas.config.objects.SerializableLocation;
...
public class PinataCommand {
    ...
    public PinataCommand(Atlas plugin) {
        this.plugin = plugin;
    ...
    public LiteralArgumentBuilder<CommandSourceStack> build() {
        return Commands.literal("pinata")
                .requires(s -> s.getSender().hasPermission("atlas.pinata"))
                .then(buildStart())
                .then(buildSpawn())
                .then(buildKillAll())
    ...
    private LiteralArgumentBuilder<CommandSourceStack> buildStart() {
        return Commands.literal("start")
                .requires(s -> s.getSender().hasPermission("atlas.pinata.start"))
    ...
    private LiteralArgumentBuilder<CommandSourceStack> buildSpawn() {
        return Commands.literal("spawn")
                .requires(s -> s.getSender().hasPermission("atlas.pinata.spawn"))
    ...
    private LiteralArgumentBuilder<CommandSourceStack> buildKillAll() {
        return Commands.literal("killall")
                .requires(s -> s.getSender().hasPermission("atlas.pinata.killall"))
                .executes(ctx -> {
    ...
    private LiteralArgumentBuilder<CommandSourceStack> buildSpawnPoint() {
        return Commands.literal("spawnpoint")
                .then(Commands.literal("add")
                        .requires(s -> s.getSender().hasPermission("atlas.pinata.spawnpoint.add"))
                        .then(Commands.argument("name", StringArgumentType.word())
                                .executes(this::handleAddSpawnPoint)))
                .then(Commands.literal("remove")
                        .requires(s -> s.getSender().hasPermission("atlas.pinata.spawnpoint.remove"))
                        .then(Commands.argument("name", StringArgumentType.word())
                                .suggests(this::suggestCentralLocations)
    ...
    private Location resolveLocation(
        ...
        sendUsage(source.getSender(), "/atlas pinata " + commandContext + " [template] [location]");
        return null;
    }
