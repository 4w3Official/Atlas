...
import dev.awe.atlas.hook.VotifierHook;
import dev.awe.atlas.hook.migration.PinataPartyMigration;
...
public class VoteCommand {
    ...
    public VoteCommand(Atlas plugin) {
        this.plugin = plugin;
    ...
    public LiteralArgumentBuilder<CommandSourceStack> build() {
        var voteNode = Commands.literal("vote")
                .requires(s -> s.getSender().hasPermission("atlas.vote"))
                .then(Commands.literal("check")
                        .then(Commands.argument("player", StringArgumentType.word())
                                .suggests(this::suggestPlayers)
                                .executes(this::handleCheck)))
                .then(Commands.literal("add")
                        .requires(s -> s.getSender().hasPermission("atlas.vote.add"))
                        .then(Commands.argument("player", StringArgumentType.word())
                                .suggests(this::suggestPlayers)
                                .then(Commands.argument("amount", IntegerArgumentType.integer(1))
                                        .executes(ctx -> handleModify(ctx, "add")))))
                .then(Commands.literal("remove")
                        .requires(s -> s.getSender().hasPermission("atlas.vote.remove"))
                        .then(Commands.argument("player", StringArgumentType.word())
                                .suggests(this::suggestPlayers)
                                .then(Commands.argument("amount", IntegerArgumentType.integer(1))
                                        .executes(ctx -> handleModify(ctx, "remove")))))
                .then(Commands.literal("set")
                        .requires(s -> s.getSender().hasPermission("atlas.vote.set"))
                        .then(Commands.argument("player", StringArgumentType.word())
                                .suggests(this::suggestPlayers)
                                .then(Commands.argument("amount", IntegerArgumentType.integer(0))
                                        .executes(ctx -> handleModify(ctx, "set")))))
                .then(Commands.literal("migrate")
                        .requires(s -> s.getSender().hasPermission("atlas.vote.migrate"))
        ...
        if (Bukkit.getPluginManager().isPluginEnabled("Votifier")) {
            voteNode.then(Commands.literal("send")
                    .requires(s -> s.getSender().hasPermission("atlas.vote.send"))
                    .then(Commands.argument("player", StringArgumentType.word())
                            .suggests(this::suggestPlayers)
