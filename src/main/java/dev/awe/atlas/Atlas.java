package dev.awe.atlas;

import dev.awe.atlas.api.event.AtlasReloadEvent;
import dev.awe.atlas.command.AtlasCommand;
import dev.awe.atlas.config.ConfigManager;
import dev.awe.atlas.handler.ActionHandler;
import dev.awe.atlas.handler.EffectHandler;
import dev.awe.atlas.hook.BetterModelHook;
import dev.awe.atlas.hook.ModelEngineHook;
import dev.awe.atlas.hook.PlaceholderAPIHook;
import dev.awe.atlas.listener.PinataListener;
import dev.awe.atlas.manager.BossBarManager;
import dev.awe.atlas.manager.DatabaseManager;
import dev.awe.atlas.manager.PinataManager;
import dev.awe.atlas.manager.VoteManager;
import dev.awe.atlas.util.Logger;
import dev.awe.atlas.util.MessageUtils;
import dev.awe.atlas.util.NamespacedKeys;
import dev.awe.atlas.util.UpdateChecker;
...
public final class Atlas extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        this.getLifecycleManager().registerEventHandler(LifecycleEvents.COMMANDS, event -> {
            AtlasCommand AtlasCommand = new AtlasCommand(this);
            event.registrar().register(AtlasCommand.createCommand("atlas"), "Main command", List.of("as"));
        });
    ...
    public boolean reload() {
        try {
            ...
            getServer().getPluginManager().callEvent(new AtlasReloadEvent());
            return true;
        } catch (Exception e) {
